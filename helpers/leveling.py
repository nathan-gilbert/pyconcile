...
def make_new_annotation(start, end, ident, annots):
    ...
    textSet = set(map(string.strip, map(string.lower, map(lambda x : x.getText(), annots))))
    if len(textSet) > 1:
...
def collapse(pairs):
    ...
    ID = 0
    for k in sortSpans(span2ids.keys()):
...
if __name__ == "__main__":
    ...
    if not options.quiet:
        print "Adding Heuristic Pairs...",
    for key in heuristicPairs.keys():
    ...
    if use_all_strings:
        for key in stringMatchPairs.keys():
        ...
        while str_matches_needed > 0:
            doc = sample(stringMatchPairs.keys(),1)[0]
            pair = sample(stringMatchPairs[doc],1)[0]
            if doc not in finalPairs.keys():
    ...
    if options.write:
        if not options.quiet:
            ...
            sys.stdout.flush()
            sys.stdout.write("\r")
            prog = ProgressBar(len(finalPairs.keys()))
            found=0
        for f in finalPairs.keys():
            DOCUMENT=f.strip()
